...
float set_SPWM1(float switching_frequency, float nominal_frequency, float speed_correction, float scale_factor) {
  ...
  if (spwm_data.steps > MAX_ARRAY_SIZE) {
    Serial.print("!!!steps: "); Serial.println(spwm_data.steps); Serial.print(" > "); Serial.print(MAX_ARRAY_SIZE);
    while (false) {
      delay(1000);
    }
  }
  spwm_data.offset = count / 2;
  spwm_data.scale_factor = scale_factor;
  for (int i = 0; i < spwm_data.steps; i++) {
    spwm_data.pwm_values[i] = trunc(0.5 + spwm_data.offset + scale_factor * spwm_data.offset * sin(2.0 * M_PI * i / spwm_data.steps)) ;
  }
...
// Interrupt routine
ISR(TIMER1_OVF_vect) {
  // load next value to comparators for SPWM...
  OCR1A = spwm_data.next_value;
  OCR1B = spwm_data.next_value;
  // strobe...
  ...
  // Prepare the next comparator value
  //increment step..
  spwm_data.step++;
  // check for overflow...
  ...
  spwm_data.next_value = spwm_data.pwm_values[spwm_data.step];
}
